...
export default function Dashboard({
  ...
  const [archiveCoverColor, setArchiveCoverColor] =
    useState("var(--honey-gold)");

  // Authentication State
  // Updated Authentication State
  const [email, setEmail] = useState("");
  const [password, setPassword] = useState("");
  const [user, setUser] = useState(null); // Tracks the authenticated Supabase user session
  const [loading, setLoading] = useState(true); // Manages loading state during initial session check
  ...
  const handleApprove = (sub) => {
    onPublish(sub);
    addToast(
      `"${sub.title}" has been approved and published to the live site!`,
      "success",
    );
  };
  ...
  const handleCompileSubmit = (e) => {
    ...
    addToast(
      `Successfully compiled and archived "${archiveEditionName}"!`,
      "success",
    );
  };
  ...
  const activities = useMemo(() => {
    const list = [
      {
        id: 1,
        text: "Elena Rostova submitted a poem 'Chasing Fireflies'",
        time: "May 20, 2026",
        type: "submit",
      },
      {
        id: 2,
        text: "Marcus Chen submitted an article 'The Rise of Algorithmic Art'",
        time: "April 15, 2026",
        type: "submit",
      },
    ];
    ...
    submissions.forEach((sub, i) => {
      if (sub.status === "Published" || sub.status === "Approved") {
        list.unshift({
          id: `act-pub-${i}`,
          text: `Dr. Sterling approved & published '${sub.title}' by ${sub.name}`,
          time: "Just now",
          type: "publish",
        });
        ...
        list.unshift({
          id: `act-rej-${i}`,
          text: `Devon Reynolds rejected '${sub.title}' by ${sub.name}`,
          time: "Just now",
          type: "reject",
        });
      } else {
        ...
        if (sub.id.startsWith("sub-17")) {
          list.unshift({
            id: `act-sub-${i}`,
            text: `${sub.name} submitted '${sub.title}'`,
            time: "Just now",
            type: "submit",
          });
        }
    ...
    return (
      <div className="auth-container fade-in-up">
        <div className="auth-card denied">
          ...
          <h2 className="auth-title denied-title" style={{ color: "#EF4444" }}>
            OUT OF REACH!
          </h2>
          <p className="auth-subtitle">Bzzzt! Swarm protection activated! 🚫</p>
          <div className="denied-box">
            <p>
              This chamber of the hive is reserved strictly for Queen Bees and
              Editor Bees. If you aren't a registered member of the Editorial
              Board, you cannot browse or manage the submission queues.
            </p>
            <p
              style={{
                marginTop: "0.75rem",
      ...
      <div className="auth-container fade-in-up">
        <div className="auth-card">
          ...
          <p className="auth-subtitle">
            Enter your board credentials to access the editorial queue.
          </p>
          ...
          >
            ...
            >
              💡 Hint for testing: Use <code>neiljamespeters12@gmail.co</code> /{" "}
              <code>MiaLikesCoffee!09</code>
            </p>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="dashboard-container fade-in-up">
      ...
      >
        <div>
          ...
          <p style={{ color: "var(--text-secondary)" }}>
            Review pending student submissions and coordinate publication
            schedules.
          </p>
        </div>
        <button
      ...
      <div className="dashboard-stats-row">
        ...
        <div
          className="dash-stat-card"
          style={{ borderLeft: "4px solid #1E5C36" }}
        >
          ...
          <div className="dashboard-main-panel">
            ...
            <div className="submissions-table-container">
              {filteredSubmissions.length > 0 ? (
                ...
                >
                  No submissions in queue matching filters.
                </p>
              )}
            </div>
          </div>
        ...
        <div className="dashboard-grid" style={{ gridTemplateColumns: "1fr" }}>
          <div className="dashboard-main-panel">
            <div style={{ marginBottom: "2rem" }}>
              ...
              >
                Review and manage active student works showing on the live
                website pages.
              </p>
            </div>
            ...
            <div className="submissions-table-container">
              {publications.length > 0 ? (
                        ...
                        <td>
                          <button
                            ...
                            onClick={() => {
                              if (
                                window.confirm(
                                  `Are you sure you want to delete the publication "${pub.title}"?`,
                                )
                              ) {
                ...
                >
                  No live publications in catalog yet. Approve student
                  applications to publish them here!
                </p>
              )}
            </div>
          </div>
        ...
        <div className="dashboard-grid" style={{ gridTemplateColumns: "1fr" }}>
          ...
          >
            <div style={{ marginBottom: "2rem", textAlign: "center" }}>
              ...
              >
                Bundle the current collection of student publications into a new
                downloadable edition archive.
              </p>
            </div>
          ...
          >
            ...
            >
              <div>
                ...
                <h4 style={{ fontSize: "1.1rem", margin: 0 }}>
                  Reviewing Attachment: {activePreview.fileName}
                </h4>
              </div>
              <button
            ...
            <div className="reader-modal-body" style={{ padding: "2rem 3rem" }}>
              ...
              >
                ...
                <div style={{ textAlign: "left" }}>
                  <p style={{ margin: 0, fontSize: "0.9rem", fontWeight: 700 }}>
                    {activePreview.name} ({activePreview.grade})
                  </p>
                  <p
                    style={{
                      margin: 0,
                  ...
                  >
                    {activePreview.email} • {activePreview.date}
                  </p>
                </div>
              </div>
              ...
              >
                ...
                {activePreview.category.replace("s", "")}
              </p>
